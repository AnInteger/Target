...
- [x] T035 [US4] 通知设置迁入：提醒总开关/每日简报时间/按目标提醒自今日页旧设置面迁至我的页通知分组，能力等价（FR-006）；今日页设置职能彻底拆除（与 T020 衔接终查）
  - ✅ 改了什么：通知组按原型 s-row 三行重排——总开关（聚合 Reminders 行 isEnabled，任一在开即开；切换全开/全关，简报无行时显式落行承载状态——否则「默认开」把聚合视图拉回 true，走查发现并修复）+ 每日简报值行（时间 + 箭头 → picker，拆行内开关）+ 按目标提醒二级展开（surfaceAlt 浅底缩进，逐行目标图标 + 频率·时间副题 + 开关，仅列活跃目标行与排程器同口径）；`_SettingsRow` 增 expanded 箭头转向；copy 新增通知组 9 键；今日页 grep 零设置残留（T020 拆除彻底）
  - ✅ 验了什么：新用例「T035 通知迁入」——总开关聚合开/逐行开关落库计数联动/总开关关全行 false 且简报行显式存在/总开关开全行 true；T026 简报断言适配新键、T034/T026 屏底断言补滚动（通知组变高挤出首屏，ListView 惰性构建）；`flutter analyze` 0 issues + `flutter test` 120/120 全绿
- [x] T036 [US4] 验收走查：结构走查（行形态全标准）+ 深浅两态对比 + V7 备份导出导入回归在新入口下通过；`flutter analyze && flutter test` 全绿
  - ✅ 改了什么：无产品码改动（纯走查）——结论归档 design/reviews.md：10 处 `_SettingsRow` 全标准（非行形态仅契约明定的账号卡/权限卡/隐私脚注）；深色态用例（platformBrightness=dark + `TargetPalette.of` 与 `TargetPalette.dark` 同实例断言）全行渲染无异常；V7 备份回归确认走数据组新入口（T026 用例既有覆盖）；附带记录 T035 总开关聚合缺口的发现与修复
  - ✅ 验了什么：新用例「T036 US4 走查」——深色令牌注入生效 + 通知三行/二级展开/数据/关于滚动全行渲染；`flutter analyze` 0 issues + `flutter test` 121/121 全绿
...
- [x] T037 [US5] 备份 v3：`lib/core/backup/backup_exporter.dart` + `lib/core/backup/backup_importer.dart`——schemaVersion 3（goals +goalType/+achievedAt、reminders +cadence、settings +nickname/avatarKey、colorKey 导出 null）；宽容解析双向（v3 读 v2 走 D3 映射重推导、v2 读 v3 忽略未知字段，contracts/backup-format.md）；`test/backup_test.dart` 增双向互导用例（往返计数一致）
