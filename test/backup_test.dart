...
Future<db.AppDatabase> _seededDb() async {
  ...
  final habit = await goals.create(
    Goal(
      id: 'g-habit',
      name: '好好吃饭',
  ...
  final milestone = await goals.create(
    Goal(
      id: 'g-ms',
      name: '发布 v1',
  ...
  await goals.addStep(
    MilestoneStep(id: 's2', goalId: milestone.id, title: '上架'),
  );

  final c1 = await checkIns.add(
    habit.id,
    const LocalDate(2026, 8, 12),
    DateTime(2026, 8, 13, 9),
  ); // 13 号补 12 号 → isBackfill
  ...
  final session = (await goals.watchSessions().first).single;
...
void main() {
  ...
  test('往返：全实体 + 设置逐字段还原（先拒绝冲突，覆盖后一致）', () async {
    ...
    await GoalRepository(target).create(
      Goal(
        id: 'local',
        name: '本地目标',
        goalType: GoalType.habit,
    ...
    expect(
      () => importer.apply(data, overwriteLocal: false),
      throwsA(isA<BackupConflictException>()),
    );
    expect(
      (await GoalRepository(target).getGoals()).length,
      1,
    ...
    expect(
      goals.firstWhere((g) => g.id == 'g-ms').deadline,
    ...
    final msMap =
        ((await BackupExporter(source)
                    .exportMap(now: DateTime.utc(2026, 8, 22)))['data']
                as Map)['goals']
            as List;
    expect(
      msMap.firstWhere((g) => g['id'] == 'g-ms'),
    ...
    expect(
      versions
          .where((v) => v.source == FrequencySource.busyMode)
          .single
          .pattern,
      const WeeklyFrequency(1),
    );
    ...
    final steps = await GoalRepository(target).stepsOf('g-ms');
    expect(steps.length, 2);
    expect(
      steps.firstWhere((s) => s.id == 's1').doneAt,
    ...
    expect(
      checkIns.firstWhere((c) => c.day == const LocalDate(2026, 8, 20)).note,
      '晚上吃得很慢，很舒服',
    );
    expect(
      checkIns.firstWhere((c) => c.day == const LocalDate(2026, 8, 12)).note,
      isNull,
    );
    ...
    expect(
      reminderRows.firstWhere((r) => r.id == 'r-goal').time,
  ...
  test('themeMode 双向宽容（004 T003·D2）：可选项往返 + 缺键/未知值 → system', () async {
    ...
    await BackupImporter(targetA).apply(
      BackupImporter(targetA).parse(BackupExporter(source).encode(mapNoKey)),
      overwriteLocal: true,
    );
    expect(
      (await SettingsRepository(targetA).get()).themeMode,
      AppThemeMode.system,
    );
    ...
    await BackupImporter(targetB).apply(
      BackupImporter(targetB).parse(BackupExporter(source).encode(mapBad)),
      overwriteLocal: true,
    );
    expect(
      (await SettingsRepository(targetB).get()).themeMode,
      AppThemeMode.system,
    );
    ...
    await SettingsRepository(targetC).update(
      (await SettingsRepository(
        targetC,
      ).get()).copyWith(themeMode: AppThemeMode.light),
    );
    final reExport = await BackupExporter(targetC)
        .exportMap(now: DateTime.utc(2026, 8, 22));
    expect(
      ((reExport['data'] as Map)['settings'] as Map)['themeMode'],
      'light',
    );
  });
  ...
  test('损坏文件：缺 data 键 / 缺实体键 / 字段类型错 → 明确报错不导入', () async {
    ...
    expect(
      () => importer.parse(exporter.encode(map2)),
      throwsA(isA<BackupFormatException>()),
    );
    ...
    expect(
      () => importer.parse(exporter.encode(map3)),
      throwsA(isA<BackupFormatException>()),
    );
  });
  ...
  test('format 头不对 / 非 JSON → 拒绝', () async {
    expect(
      () => BackupImporter(target).parse('not json'),
      throwsA(isA<BackupFormatException>()),
    );
    expect(
      () =>
          BackupImporter(target)
              .parse('{"format":"other","version":1,"data":{}}'),
      throwsA(isA<BackupFormatException>()),
    );
  });
  ...
  test('导出文件头：format/version/exportedAt + 文件名（FR-015）', () async {
    ...
    expect(
      ...
      'Target-备份-20260822.targetbackup',
    );
  });
  ...
  test('v4 双向（新读旧）：v1 三分支 kind → D3 映射 + cadence/nickname 缺省（T037）', () async {
    ...
    final goals = await GoalRepository(target).getGoals();
    expect(goals.firstWhere((g) => g.id == 'g-habit').goalType, GoalType.habit);
    expect(
  ...
  test('v4 双向（往返）：goalType/achievedAt/cadence/nickname/avatarKey/colorKey null（T037）', () async {
  ...
  test('v5 双向：目标规划、里程碑顺序与评分算法边界完整保留', () async {
    ...
    await settings.update(
      const Settings(
        defaultShortCadenceDays: 8,
        defaultLongCadenceDays: 16,
    ...
    final target = db.AppDatabase(NativeDatabase.memory());
    ...
    expect(restoredSettings.defaultShortCadenceDays, 8);
    expect(restoredSettings.defaultLongCadenceDays, 16);
    expect(
      restoredSettings.scoreAlgorithmStartedOn,
