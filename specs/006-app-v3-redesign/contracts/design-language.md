...
- Liquid Glass 档位（R1 裁定后）：
  - **dock**（底栏双 tab 胶囊，R2 定稿＝参照 iOS 健康 floating tab bar）：
    与记录钮**同材质浅玻璃**（shell 档）· blur 24 · saturate 180%/160% ·
    圆角 30px 近胶囊；**选中效果 = 蓝色着色**（图标+文字 accent 蓝、文字 600），
    **无底色高亮块**；未选中 on-surface。R1 的深蓝胶囊方案被 R2 反转作废。
