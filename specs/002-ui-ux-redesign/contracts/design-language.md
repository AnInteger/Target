...
- 2026-08-20（今日屏 R4 评审驱动）——浅色主题整体调校（深色不动）：
  - **底幕再调淡**：bg-grad 四档 `#f9edf4/#f5e8f6/#f0e2f8/#e9dbf2 → #f9eff4/#f7ecf5/#f4ebf7/#f1eaf8`，均值 `#f1e5f4 → #f4eff6`（末档薰衣草过深发闷，向定稿参照低饱和暖粉回归）。
  - **强调色墨梅化**：`--accent #17181c（纯黑）→ #252230（墨梅）`；随族微调 `--on-surface #1d1a24`、`--on-surface-variant #625e70`、`--divider #e8e3f0`、`--surface-alt #f5f0f9`。原则：主操作强调不取纯黑（#000 系），一律带品牌紫相的墨色，深浅两主题各自保持"反色实心"语义。
  - **shadow-low 弥散化**：浅色 `0 1px 3px → 0 2px 10px`（同为 6% 不透明度）——底幕变淡后白卡分层由"贴边硬影"改"弥散软影"，与参照卡片的轻阴影一致。
