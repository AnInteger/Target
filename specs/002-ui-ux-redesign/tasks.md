...
- [ ] T008 [US1] "今日"屏原型评审：`design/reviews.md` 记录结论至"通过"（**门禁：通过后 US2 实现任务方可开工**）（R1 六项 + R2 九项 + R3 自审六项已全改；2026-08-20 R4 四项已改——年度守护卡移除、导航条参照重做（inset 对齐/全圆角胶囊/图标等大）、浅色墨梅 accent、底幕渐变按参照图实测锚点可见化（粉→紫 135deg），R4 待评审）
