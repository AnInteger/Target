...
- [x] T023 [US3] 全部目标列表页：lib/app/router.dart 新增 /goals-all（today 分支子路由）+ lib/features/goals/goals_all_view.dart（新文件，按冻结稿 v2-goals-all 实现：全部+各小类单选筛选、筛选空态说明、进详情/编辑/暂停恢复/删除全动线）（depends T011 冻结, T022）
  - ✅ 实现完成：goals_all_view.dart 过渡页全量换装冻结稿——_TopBar（38 圆返回钮同详情 .dt-btn + titleM 标题 + goalsAllCount 计数随筛选联动 + 新建胶囊 surface/divider/accent 加号 → /goal-editor）+ _FilterRow 横滚单选 chips（全部无点 + 十小类 8px 大类色点；浅色选中墨底反白点反白、深色选中 surface 底 on-surface 描边，700 字重）+「全部」视图 _groupedByMajor 三大类分组（10px 色点 ghead + 计数，组间 s3+s2）/筛选视图平铺 + _GoalCard（44px surfaceAlt rMd 大类色图标格 + 名称 + _StatusBadge 状态优先：已暂停 warning/已达成 positive/活跃=类型，archived 遗留态按暂停口径 + 摘要行口径同详情 meta 延展：达成=MM-DD 照面/暂停=historyCountMeta/短期=倒计时·已完成 N%（stepsProvider 里程碑占比）+ 6px 大类色 prog 条/习惯长期=连击→今天已记录→还没有记录 + › 尾）+ 长按 _showManageSheet（grab + caption 图标名 + 编辑›/暂停|恢复 canTransitTo 显隐/删除 danger，行先 pop 再执行同详情 _MenuRow 口径）+ _confirmDelete 复用 goalDeleteDialog 双胶囊二次确认物理级联 + _FilterEmpty（88px 圆底 flag 图形 + 分类名明示 body + 新建 CTA；全库空复用今日空态文案）；goalsAllRow-{id} 行 key 契约保留（tap 进详情）；copy 新增 goalsFilterAll/goalsNewCapsule/goalsFilterEmptyTitle/Body/goalAchievedMeta/goalRecordedTodayMeta/shortTermProgressMeta
  - ✅ flutter analyze → "No issues found!"；flutter test → "All tests passed!"（150 全绿 = 149 +T023 筛选/空态/长按管理全动线用例；goalsAllList/goalsAllFilter-*/goalsAllMenu-*/goalsAllEmptyCta/goalsAllCount 锚点；编辑器返回键无 Tooltip pageBack 不达，byIcon chevron 定位）
...
- [x] T028 [US5] 回顾页头部与周平均区块：lib/features/review/review_view.dart 重做——小字语义位 + 大标题「回顾」+ 周切换入口 + 完成率环与环比（depends T002, T027）
  - ✅ 实现完成：review_view.dart 全文件重写为 v2 三区块之头部/周切换/概览（T027 引擎三法接线）——_Head 顶栏（padding-top s4 + displayL「回顾」挂 screenTitle + 右上 38px 日历圆钮 surface+divider+shadowLow，Tooltip「选择周期」占位无动作）；_WeekNav 周切换行（‹ › 30px 圆钮 shadowLow + 居中周区间「M 月 D 日 – M 月 D 日」bodyM tabular 挂 weekRange；前瞻钳制——本周时 › 置灰 0.35 不可点，未来周无可回看；状态 _selectedWeek 可无限回翻）；_OverviewCard 本周概览卡（titleS 题款 + surface/radiusLg/shadowLow/padding s5：displayL tabular 大数字 N% + 环比胶囊「↑ N%/↓ N%」（升=positiveFill 底 positiveOn 字、降=surfaceAlt 底 warning 字，FR-013 双编码）+ 副行「周平均完成率 · 上周 N%/无可比较」+ 72px 均分环 _RingPainter r30/stroke7/divider 底轨/positive 圆头弧/环心 titleS 数字+labelS 均分；零应记整卡「—」×2 +「该周暂无记录」+ 空轨 + 无胶囊）。空态引导暂沿 003 _EmptyState 形态（该周无任何应记目标时出列，T029 换 v2 空周画板）。003 卡片轮播/三态图例/四周走势梯/观察语三档全数退役。copy：reviewTitle→「回顾」+ 新 reviewOverviewTitle/reviewAvgSub(int?)/reviewAvgEmpty/reviewAvgScore/reviewPickWeek − 退役 reviewWeekSum/Legend×3/TrendCap/Coach×3。测试：T021 重写为 v2 周切换走查（默认本周 0%/↓36 → ‹ 上周 36%/↑36/环心 36/上周 0% → › 回本周 + 前瞻钳制点不动）；T031 重写（概览整卡空态「—」×2 + 无胶囊 + CTA ≤1 交互直达编辑器，003 竖直居中断言随列表流退役）
  - ✅ flutter analyze → "No issues found!"；flutter test → "All tests passed!"（159/159）
- [x] T029 [US5] 每日活动与本周目标区块：lib/features/review/review_view.dart 续——七天圆点行（M–S 语义、按完成度着色、未来日不完成态）+ 本周目标列表（目标名+线性进度+百分比）+「查看全部」入口 + 空态引导 CTA 直达编辑器（depends T028）
  - ✅ 实现完成：三区块收口——_DaysCard 每日活动（surface/radiusLg/shadowLow/padding s5 卡内 Row spaceBetween 七列：lbl 星期简称 labelS（今日 accent+w700）/36px 圆点三档（full=positiveFill 实底+positiveOn 对勾 16、partial=surfaceAlt 底+3px positiveFill 描边圈、none=surfaceAlt——FR-013 图形双编码）/下标 n=当日全量打卡次数 labelS tabular（0→「·」，未来日引擎恒零着色计数不落）；_GoalsSection 本周目标（题款 titleS+Spacer+「查看全部 ›」accent→push /goals-all；_GoalWeekCard=42px radiusMd surfaceAlt 图标格大类色 icon+bodyL 名+6px 大类色 FractionallySizedBox 进度（weekRateOf.fraction，track surfaceAlt）+x/y bodyS tabular+chev，整卡→push /goal/{id}，key reviewGoal-{id}；只列该周 expectedDays>0 的目标（状态不滤回看口径））；_EmptyState v2 换装（96px surfaceAlt 圆底+history_edu 40+titleM「这一周还没有记录」+双行引导语+保留「新建目标」CTA——冻结稿不带钮但 FR-007/SC-004 ≤1 交互契约要求动线；003 七格空圈+_DashedCirclePainter 退役）。copy：+reviewDaysTitle/reviewGoalsTitle，reviewEmptyTitle/Sub 换 v2 冻结稿文案。测试：T021 扩三区块断言（本周「·」×7 无对勾+两卡在场；‹ 上周「·」×2 全 partial 无对勾+4/7、1/7 手工核算）；新增 T029 专项（唯一 full 今日对勾+n 计数 1/2/「·」×5+今日列首 Text w700+卡→GoalDetailPage→chevron 返回+查看全部→goalsAllList+dock 两页签恒定）
  - ✅ flutter analyze → "No issues found!"；flutter test → "All tests passed!"（160/160，+1）
