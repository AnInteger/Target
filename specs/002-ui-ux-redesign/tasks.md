...
- [x] T028 [US5] 品牌母版与 iOS 资产 `design/brand/` + `pubspec.yaml`：Target 图标 1024 母版（浅/深）→ 配置 `flutter_launcher_icons` + `flutter_native_splash`（dev_dependencies）→ `dart run` 生成 iOS 全档资产（图标/启动屏）✅ 2026-08-22 用户裁定 A 案「今日之环」（原话「1.选择A」）→ icon-a-light/dark.svg 母版（几何 1:1 派生 proposal）→ 1024² PNG 像素级核验（自写解码器逐点对照；坑：SVG 注释含 `--` 非法 XML，light 首渲全白报废重渲）→ 双工具生成全绿（AppIcon 全档 + LaunchImage 浅深双套 + storyboard 重构）→ 手工补 iOS 18 深色图标变体（AppIconDark + appearances 条目；tinted 未做回退深色）→ analyze 0 + 69 测试绿；真机生效需再触发一次构建（明细 reviews.md 品牌图标 2026-08-22 节）
...
- [x] T033 发布验证：`flutter analyze` + `flutter test` 全绿 → `git push` → Codemagic 绿 → 真机核对（quickstart 阶段 C：图标/启动屏/小组件/深色/动效帧率/通知时机与文案）✅ 2026-08-22 用户确认侧载成功归档（原话「2.侧载成功，可以归档」）：agent 侧 analyze 0 + 69 测试绿、45 提交 push（4666294..24277b5）；用户手动侧 Codemagic `ios-unsigned` 构建绿 → unsigned.ipa iLoader 自签侧载真机运行正常——**T029 小组件首次 Swift 编译随真机实证**（无 Xcode 挂起点解除）；注：本轮真机图标/启动屏仍为默认（T028 当时尚未过链），A 案资产进链后需再触发一次构建侧载方可在真机看到（明细 reviews.md 实现审计 T033 节）
